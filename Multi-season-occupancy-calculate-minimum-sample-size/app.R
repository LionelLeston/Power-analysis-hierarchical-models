...
ui <- fluidPage(
    ...
    # Application title
    titlePanel("Minimum Sample Size for a Multi Season Occupancy Survey Design"),
...
server <- function(input, output) {
  ...
  calcSFormula <- function(K1,K2,p1,p2,psi1,psidiff,alpha,pow)
  {
    psi2 <-psi1*(1-psidiff)
    pp1 <- 1-(1-p1)^K1
    pp2 <- 1-(1-p2)^K2
    ...
    output$estimatedPower<-renderText({
      paste0("Minimum sample size (based on Guillera-Arroita and Lahoz-Monfort 2012): ", glimpse(calcSFormula(K1=input$K1,
                                                          K2=input$K2,
                                                          p1=input$p1,
                                                          p2=input$p2,
                                                          psi1=input$psi1,
