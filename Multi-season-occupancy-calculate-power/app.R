...
ui <- fluidPage(
    ...
    # Application title
    titlePanel("Calculate Power of a Multi Season Occupancy Survey Design"),
...
server <- function(input, output) {
    ...
    calcPowerFormula <- function(S1,S2,K1,K2,p1,p2,psi1,psidiff,alpha) 
    {
      psi2 <-psi1*(1-psidiff)
      pp1 <- 1-(1-p1)^K1
      pp2 <- 1-(1-p2)^K2
    ...
    #calculate power to detect difference in occupancy based on survey design and alpha
    output$estimatedPower<-renderText({
      paste0("Power of survey design (based on Guillera-Arroita and Lahoz-Monfort 2012): ", glimpse(calcPowerFormula(S1=input$S1,
                                                          S2=input$S2,
                                                          K1=input$K1,
                                                          K2=input$K2,
                                                          p1=input$p1,
